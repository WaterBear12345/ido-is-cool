...
(async () => {
  ...
  check("the tab bar is slim", g.navBottom - g.navTop <= 56, g.navBottom - g.navTop);
  eq("the status bar is solid, so nothing scrolls under it", await page.getAttribute('meta[name="apple-mobile-web-app-status-bar-style"]', "content"), "black");
